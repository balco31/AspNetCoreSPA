...
namespace SecurityLib.Session
{
    ...
    public class CustomSessionMiddleware
    {
            ...
            IOptions<SessionOptions> options)
        {
            ...
            _dataProtector = dataProtectionProvider.CreateProtector(nameof(CustomSessionMiddleware));
            _options = options.Value;
            _sessionStore = sessionStore;
        }
        
        private Tuple<string, bool> getSessionKeyInfo(HttpContext context)
        {
            string sessionKey = string.Empty;
            try
            {
                if (context.Request.Query.Keys.Any(a => a == "session"))
                {
                    // Case when sessionKey exists as claim info in cookie ticket after user is authenticated. This is not a new one already exists
                    var sessionFromUrl = context.Request.Query.FirstOrDefault(f => f.Key == "session").Value;
                    var ticket = CookieProtection.ReadAuthCookie(context, sessionFromUrl, _logger);
                    var claims = ticket.Principal.Claims;
                    sessionKey = claims.FirstOrDefault(f => f.Type == "sessionKey").Value;
                    return new Tuple<string, bool>(sessionKey, false);
                }
                else if (context.Request.Headers.Any(a => a.Key == "sessionKey"))
                {
                    // Case sessionKey is posted from local web page which its posts the authentication info. Because this post authentication may be in 2 steps
                    // the first time we return true which means a new session. The second time we respond false in second tuple param which means an existent one session
                    sessionKey = context.Request.Headers.FirstOrDefault(f => f.Key == "sessionKey").Value;
                    return new Tuple<string, bool>(sessionKey, context.Request.Headers.FirstOrDefault(f => f.Key == "isNew").Value == "true" ? true : false);
                }
            ...
            // Case we have not any session yet (another app posts in our app to authenticate in one step) so we create a new key and we respond back
            // as true in order handler below to create a new session
            var guidBytes = new byte[16];
            CryptoRandom.GetBytes(guidBytes);
            sessionKey = new Guid(guidBytes).ToString();
            context.Request.Headers.Append("sessionKey", sessionKey); //we respond also to request header the SessionKey in order to be used from page (store in cookie)
            return new Tuple<string, bool>(sessionKey, true);
        }
        ...
        private class SessionEstablisher
        {
            ...
            private void SetCookie()
            {
                ...
                //_context.Response.Cookies.Append(_options.Cookie.Name, _cookieValue, cookieOptions); //old code
                _context.Response.Headers[_options.Cookie.Name] = _cookieValue; //new code
